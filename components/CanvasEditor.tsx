...
export const CanvasEditor: React.FC<CanvasEditorProps> = ({
  ...
  const width = 1080;
  const height =
    config.size === "1080x1350"
      ? 1350
      : config.size === "1080x1920"
        ? 1920
        : 1080;
  const aspectRatio = width / height;

  const handleLayerAction = useCallback(
    (id: string, action: "visible" | "locked") => {
      setConfig((prev) => ({
        ...prev,
        layers: (prev?.layers || []).map((l) => {
          if (l.id !== id) return l;
          if (action === "visible")
            return { ...l, visible: l.visible === false };
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      if (draggingId) {
        ...
        setConfig((prev) => ({
          ...prev,
          layers: (prev?.layers || []).map((l) =>
            l.id === draggingId ? { ...l, position: { x, y } } : l,
          ),
        }));
      }
  ...
  return (
    <div className="flex h-full w-full overflow-hidden bg-[#F1F5F9] relative select-none">
      ...
      <div className="w-16 bg-white border-r border-slate-200 flex flex-col items-center py-6 gap-6 z-30 shadow-sm">
        <button className="p-3 text-op7-blue bg-op7-blue/10 rounded-xl transition-all shadow-sm">
          <MousePointer2 size={22} />
        </button>
        <button className="p-3 text-slate-400 hover:text-op7-navy hover:bg-slate-50 rounded-xl transition-all">
          <Type size={22} />
        </button>
        <button className="p-3 text-slate-400 hover:text-op7-navy hover:bg-slate-50 rounded-xl transition-all">
          <ImageIcon size={22} />
        ...
        <button
          onClick={onUndo}
          disabled={!canUndo}
          className={`p-3 rounded-xl transition-all ${canUndo ? "text-slate-600 hover:bg-slate-50" : "text-slate-200 cursor-not-allowed"}`}
        >
          <Undo2 size={18} />
        </button>
        <button
          onClick={onRedo}
          disabled={!canRedo}
          className={`p-3 rounded-xl transition-all ${canRedo ? "text-slate-600 hover:bg-slate-50" : "text-slate-200 cursor-not-allowed"}`}
        >
          <Redo2 size={18} />
        </button>
      </div>
      ...
      <div className="flex-1 relative flex flex-col items-center justify-center p-20 overflow-hidden bg-[#f3f4f6]">
        ...
        {showTooltip && (
          <div className="absolute top-28 left-1/2 -translate-x-1/2 bg-op7-navy text-white px-6 py-3 rounded-2xl shadow-premium z-[110] animate-in slide-in-from-top-4 flex items-center gap-3 border border-white/10">
            ...
            <p className="text-[10px] font-black uppercase tracking-widest">
              Editor Ativo • Dê duplo-clique no texto para editar
            </p>
          </div>
        )}
        ...
        <div className="absolute top-8 left-1/2 -translate-x-1/2 flex items-center gap-3 bg-white/90 backdrop-blur-2xl border border-white p-2 rounded-2xl shadow-premium z-30">
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 hover:bg-op7-blue/5 text-op7-navy font-black text-[10px] uppercase tracking-widest rounded-xl transition-all border border-transparent hover:border-op7-blue/10"
          >
          ...
          <button
            ...
            className="flex items-center gap-2 px-3 py-2 hover:bg-slate-50 text-slate-500 font-bold text-[10px] uppercase tracking-widest rounded-xl transition-all"
          >
            <Type size={14} /> Texto
          </button>
          <button
            onClick={() => onRegenerate("art")}
            className="flex items-center gap-2 px-3 py-2 hover:bg-slate-50 text-slate-500 font-bold text-[10px] uppercase tracking-widest rounded-xl transition-all"
          >
        ...
        >
          ...
          >
            {/* Background Layer */}
            <div
              className={`absolute inset-0 transition-all ${selectedLayerId === "background" ? "ring-4 ring-inset ring-op7-blue/30" : ""}`}
              onMouseDown={(e) => {
                e.stopPropagation();
            ...
            {/* Guides */}
            {snapLines.x !== null && (
              <div className="absolute top-0 bottom-0 left-1/2 w-[1px] bg-op7-blue/50 z-[100] pointer-events-none" />
            )}
            {snapLines.y !== null && (
              <div className="absolute left-0 right-0 top-1/2 h-[1px] bg-op7-blue/50 z-[100] pointer-events-none" />
            )}
            ...
            {config?.layers?.map((layer) => {
              ...
              return (
                ...
                >
                  <div
                    className={`relative w-full group transition-all ${isSelected ? (isEditing ? "" : "ring-2 ring-op7-blue shadow-2xl") : "hover:ring-1 hover:ring-op7-blue/20"}`}
                  >
                    {layer?.type === "text" && (
                      <div
                    ...
                    {layer?.type === "button" && (
                      <div
                        ...
                        style={{
                          ...
                          letterSpacing:
                            layer?.style?.letterSpacing || "0.05em",
                          textTransform: (layer?.style?.textTransform ||
                            "uppercase") as any,
                          transform: `rotate(${layer?.style?.rotate || 0}deg)`,
                          opacity: layer?.style?.opacity ?? 1,
          ...
          <div className="absolute inset-0 bg-white/60 backdrop-blur-[4px] z-[120] flex flex-col items-center justify-center p-8 animate-in fade-in duration-500">
            <div className="w-full max-w-sm bg-white p-8 rounded-[40px] shadow-premium border border-slate-100 flex flex-col items-center gap-6">
              <div className="relative">
                ...
                <Sparkles
                  className="absolute inset-0 m-auto text-op7-blue animate-pulse"
                  size={24}
                />
              </div>
              <div className="text-center">
                <h4 className="text-lg font-black text-op7-navy uppercase tracking-tighter mb-1">
                  {progress.step}
                </h4>
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em]">
                  Otimizando Design • {progress.percentage}%
                </p>
              </div>
              <div className="w-full h-1.5 bg-slate-100 rounded-full overflow-hidden">
                <div
                  className="h-full bg-op7-blue transition-all duration-700"
      ...
      <div className="w-80 bg-white border-l border-slate-200 flex flex-col z-30 shadow-sm">
        <div className="flex border-b border-slate-100 p-2">
          <button
            ...
            className={`flex-1 py-3 text-[10px] font-black uppercase tracking-widest flex items-center justify-center gap-2 rounded-xl transition-all ${sidebarTab === "props" ? "bg-op7-blue/5 text-op7-blue" : "text-slate-400 hover:text-slate-600"}`}
          >
            <Settings2 size={14} /> Propriedades
          </button>
          <button
            onClick={() => setSidebarTab("layers")}
            className={`flex-1 py-3 text-[10px] font-black uppercase tracking-widest flex items-center justify-center gap-2 rounded-xl transition-all ${sidebarTab === "layers" ? "bg-op7-blue/5 text-op7-blue" : "text-slate-400 hover:text-slate-600"}`}
          >
        ...
        <div className="flex-1 overflow-auto px-6 py-8 space-y-8 h-full custom-scrollbar">
          {sidebarTab === "layers" ? (
            <div className="space-y-4">
              <h3 className="text-[10px] font-black text-op7-navy uppercase tracking-widest opacity-40">
                Estrutura Visual
              </h3>
                ...
                {config?.layers?.map((layer) => (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <div
                        className={`w-9 h-9 rounded-xl flex items-center justify-center transition-colors ${selectedLayerId === layer?.id ? "bg-op7-blue text-white shadow-lg shadow-op7-blue/20" : "bg-white text-slate-400 border border-slate-200"}`}
                      >
                        {layer?.type === "text" ? (
                          <Type size={16} />
                        ) : layer?.type === "button" ? (
                      ...
                      <div>
                        <p
                          className={`text-[11px] font-black uppercase tracking-tight ${selectedLayerId === layer?.id ? "text-op7-navy" : "text-slate-600"}`}
                        >
                          {layer?.name || "Camada S/N"}
                        </p>
                    ...
                    <div className="flex items-center gap-1">
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                          handleLayerAction(layer?.id, "visible");
                        }}
                        className="p-2 hover:bg-white rounded-lg transition-colors text-slate-400 hover:text-op7-blue"
                      >
                        {layer?.visible !== false ? (
                          <Eye size={14} />
                        ) : (
                      ...
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                          handleLayerAction(layer?.id, "locked");
                        }}
                        className={`p-2 hover:bg-white rounded-lg transition-colors ${layer?.locked ? "text-op7-accent" : "text-slate-400 hover:text-op7-blue"}`}
                      >
                        {layer?.locked ? (
                          <Lock size={14} />
                        ) : (
            ...
            <div className="space-y-8 animate-in fade-in slide-in-from-right-4 duration-300">
              <DiagnosticProps
                layer={
                  config?.layers?.find((l) => l.id === selectedLayerId) ||
                  (null as any)
                }
                setConfig={setConfig}
            ...
            <div className="flex flex-col items-center justify-center py-20 text-center gap-4">
              ...
              <div>
                <h4 className="text-xs font-black text-op7-navy uppercase tracking-widest mb-1">
                  Nada Selecionado
                </h4>
                <p className="text-[10px] text-slate-400 font-medium px-4">
                  Selecione um elemento no canvas para ajustar suas
                  propriedades.
                </p>
              </div>
            </div>
...
const DiagnosticProps: React.FC<{
  layer: Layer;
  setConfig: any;
  isBackground?: boolean;
  palette?: any;
}> = ({ layer, setConfig, isBackground, palette }) => {
  if (isBackground)
    return (
              ...
              {palette &&
                ...
                Object.entries(palette).map(([name, hex]: [string, any]) => (
                  <button
                    key={name}
                    onClick={() =>
                      setConfig((p: any) => ({ ...p, backgroundColor: hex }))
                    }
          ...
          <div className="space-y-1.5 col-span-2">
            ...
            <div className="flex items-center gap-3">
              <input
                ...
                min="-180"
                max="180"
                value={layer?.style?.rotate || 0}
                onChange={(e) =>
                  updateStyle({ rotate: parseInt(e.target.value) })
                }
                className="flex-1 accent-op7-blue"
              />
              <span className="text-[10px] font-black text-slate-400 w-10 text-right">
                {layer?.style?.rotate || 0}°
              </span>
            </div>
          </div>
          <div className="space-y-1.5 col-span-2">
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-4">
              ...
              <>
                <div className="space-y-1.5">
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    <input
                      type="color"
                      value={layer?.style?.backgroundColor || "#000000"}
                      onChange={(e) =>
            ...
            {(layer?.type === "button" || layer?.type === "image") && (
              <div className="space-y-1.5">
                <label className="text-[9px] font-bold text-slate-400 uppercase">
                  Arredondamento: {layer?.style?.borderRadius || 0}px
                </label>
                <input
                  type="range"
                  min="0"
                  ...
                  className="w-full accent-op7-blue"
                />
              </div>
            )}
          </div>
